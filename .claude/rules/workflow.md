# Workflow & Task Management

이 문서는 Unhinged Students 프로젝트의 개발 워크플로우와 작업 관리 규칙을 정의합니다.

## 백로그로 작업 목록 관리

모든 작업은 `backlog/` 디렉토리의 파일에 기록하고 관리합니다.

**백로그 파일 관리 규칙:**
- 파일명: `yyyymmdd-hhmmss.md` (예: `20260118-154211.md`)
- 새로운 작업 세트를 시작할 때 새 백로그 파일 생성
- 작업 완료 시 체크박스 체크 또는 완료 표시
- 커밋하기 전에 반드시 백로그 업데이트
- 작업 중 발견된 추가 작업도 즉시 백로그에 기록

**백로그 형식 예시:**
```markdown
# [작업 제목]

**생성일:** 2026-01-18
**상태:** 진행 중

## 진행 중
- [ ] 사용자 인증 기능 구현
- [ ] 게임 메인 화면 UI

## 완료
- [x] 프로젝트 초기 설정
- [x] 개발 가이드라인 작성

## 대기 중
- [ ] 점수 시스템 구현
- [ ] 멀티플레이어 기능
```

**workspace 연동:**
- 백로그 생성 시 workspace 루트의 `WORKLOG.md`에도 링크 추가
- 대규모 작업은 WORKLOG.md에 계획 먼저 수립 후 백로그 생성

## 점진적 개발 (Incremental Development)

복잡한 기능은 한 번에 만들지 않고 작은 단위로 나누어 개발합니다.

**점진적 개발 프로세스:**
1. **기능 분해:** 큰 기능을 작고 독립적인 단위로 분해
2. **최소 구현:** 가장 작은 동작하는 단위를 먼저 구현
3. **테스트 & 커밋:** 작은 단위마다 테스트하고 커밋
4. **플레이테스트:** 실제로 실행하여 동작 확인
5. **피드백 요청:** 사용자에게 의견을 구하고 다음 단계 결정
6. **반복:** 피드백을 반영하여 다음 단위 구현

**예시:**
- ❌ 나쁜 예: "완전한 인벤토리 시스템을 한 번에 구현"
- ✅ 좋은 예:
  1. 먼저 아이템을 화면에 표시
  2. 플레이테스트 후 피드백
  3. 아이템 클릭 기능 추가
  4. 플레이테스트 후 피드백
  5. 아이템 사용 기능 추가
  6. 계속 점진적으로 확장

**장점:**
- 빠른 피드백으로 방향 수정 가능
- 작은 단위로 테스트하여 버그 발견 용이
- 각 단계마다 동작하는 코드 유지
- 오버엔지니어링 방지

## 커밋 워크플로우

테스트가 모두 통과한 후에만 변경사항을 커밋하고 푸시합니다.

**커밋 워크플로우:**
1. ✅ 모든 테스트 통과 확인
2. **백로그 업데이트** (완료된 작업 체크, 새로운 작업 추가)
3. 변경사항 스테이징 (`git add`)
4. 의미 있는 커밋 메시지 작성
5. 커밋 생성
6. 원격 저장소에 푸시

**금지 사항:**
- ❌ 백로그 업데이트 없이 커밋
- ❌ 피드백 없이 다음 기능으로 진행

## 전체 워크플로우

```
1. 백로그에 작업 추가
   ↓
2. 작은 단위로 기능 분해
   ↓
3. 코드 작성 (최소 단위)
   ↓
4. 테스트 작성
   ↓
5. 테스트 실행
   ↓
6. ✅ 통과 → 7번으로
   ❌ 실패 → 3번으로 돌아가서 수정
   ↓
7. 플레이테스트
   ↓
8. 백로그 업데이트
   ↓
9. 커밋 & 푸시
   ↓
10. 사용자에게 피드백 요청
   ↓
11. 피드백 반영하여 다음 단위로 (1번으로)
```

## 핵심 가이드라인

✅ **해야 할 것:**
- 작은 단위로 나누어 개발
- 테스트 먼저 작성하고 통과 확인
- 백로그를 항상 최신 상태로 유지
- 각 단계마다 플레이테스트
- 사용자 피드백을 적극 수용

❌ **하지 말아야 할 것:**
- 복잡한 기능을 한 번에 구현
- 테스트 없이 커밋
- 백로그 업데이트 없이 커밋
- 피드백 없이 다음 기능으로 진행
