 ...
 */
function processLaserDamage(options: LaserDamageOptions): LaserDamageResult {
    const { attackerId, x1, y1, x2, y2, damage, respawnDelay } = options;
...
export function registerLaserHandlers(socket: TypedSocket, io: TypedServer): void {
    ...
    socket.on('laserAiming', (data: LaserAimingData) => {
    ...
    // Handle laser attack
    socket.on('laserAttack', (data: LaserAttackData) => {
