...
- [ ] 기본 WebSocket 서버 구현 (포트: 3000)
- [ ] 서버 시작 스크립트 작성
- [ ] 서버 연결 테스트

#### 클라이언트 구현
- [ ] Socket.io 클라이언트 라이브러리 추가 (CDN)
- [ ] 네트워크 모듈 생성 (src/network.js)
- [ ] 서버 연결 로직 구현
- [ ] 플레이어 ID 할당 및 관리
- [ ] 연결/해제 이벤트 핸들링
...
- [ ] 자신의 위치를 서버로 전송 (주기적)
- [ ] 다른 플레이어 위치 수신 및 저장
- [ ] 다른 플레이어 렌더링 (RemotePlayer 클래스)
- [ ] 플레이어 목록 관리 (추가/제거)
- [ ] 보간(interpolation)으로 부드러운 움직임 구현

#### 테스트 및 최적화
- [ ] 멀티 브라우저 동시 테스트
- [ ] 위치 동기화 정확도 확인
- [ ] 네트워크 지연 처리
- [ ] 테스트 페이지 작성 (test/multiplayer.test.html)

**성공 기준**: 두 명이 같은 맵에서 움직이는 것을 확인
