...
declare const global: { isKeyPressed: Mock };
global.isKeyPressed = jest.fn().mockReturnValue(false) as Mock;
...
interface AttackArea {
    x: number;
    y: number;
    radius: number;
}

interface KnockbackEndPosition {
    x: number;
    y: number;
}
...
class Character {
    ...
    startKnockback(_attackerX: number, _attackerY: number, endX: number, endY: number): void {
...
describe('Knockback System', () => {
    ...
    describe('calculateKnockbackEndPosition', () => {
        test('should calculate end position in knockback direction', () => {
            const result = Character.calculateKnockbackEndPosition(
                0,
                100,
                100,
                100,
...
describe('Image Loading System', () => {
    ...
    class CharacterWithImageLoading extends Character {
        _lastImageCallbacks: ImageLoadCallbacks | null = null;
        ...
        loadImage(path: string, retries = 3): Promise<void> {
    ...
    describe('retry logic', () => {
        ...
        test('should load fallback after all retries exhausted', async () => {
            ...
            char.loadImage = jest.fn().mockImplementation((path: string, retries = 3) => {
                if (retries === 0) {
