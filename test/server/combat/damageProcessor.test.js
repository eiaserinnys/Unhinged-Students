...
const mockValidation = {
    ...
    calculateKnockbackEndPosition: jest.fn((ax, ay, tx, ty, dist) => ({
        x: tx + dist,
        y: ty,
    })),
};
...
describe('DamageProcessor', () => {
    let processAreaDamageToPlayers;
    let processAreaDamageToDummies;
    ...
    describe('broadcastDamageEvents', () => {
        ...
        it('should use custom event names when provided', () => {
            const hitPlayers = [{ playerId: 'victim-1' }];
            const hitDummies = [{ dummyId: 'dummy-1' }];
