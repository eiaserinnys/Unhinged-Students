...
const mockValidation = {
  isValidNumber: jest.fn((val) => typeof val === 'number' && Number.isFinite(val)),
  clampCoordinates: jest.fn((x, y) => ({ x, y })),
  calculateDistance: jest.fn((x1, y1, x2, y2) => Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)),
  calculateKnockbackDistance: jest.fn(() => 50),
  calculateKnockbackEndPosition: jest.fn((ax, ay, tx, ty, dist) => ({ x: tx + 50, y: ty })),
  lineCircleIntersect: jest.fn(() => false),
};

// Mock game state
const mockPlayers = new Map();
const mockDummies = new Map();
...
describe('CombatHandler Security', () => {
  ...
  describe('telepathyDamage - Tick Rate Validation', () => {
    ...
    it('should reject tick at 80% of interval (below tolerance)', () => {
      ...
      player.telepathyLastTickTime = Date.now() - Math.floor(tickInterval * 0.80);
